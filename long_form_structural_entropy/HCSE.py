...
class PartitionTree():
    ...
    def __init__(self,adj_matrix):
        ...
        self.build_leaves()
        # 标记孤立节点
        self.isolated_nodes = [i for i, vol in enumerate(self.node_vol) if vol == 0]
    ...
    def path_entropy(self, id, ent):
        if self.tree_node[id].vol == 0 or id in [self.leaves[i] for i in self.isolated_nodes]:
            return -1e-6  
...
def main(bipartite_adj):
    # Initialize the structural entropy value dictionary
    claims_node_entropy = {}
    # Convert the input upper-right subgraph to a numpy array
    bipartite_adj = np.array(bipartite_adj)
    # Get the number of claims (m) and statements (n)
    m, n = bipartite_adj.shape
    
    whole_bipartite_adj = np.block([
        [np.zeros((m, m)), bipartite_adj],       
        [bipartite_adj.T, np.zeros((n, n))]     
    ])
    print(whole_bipartite_adj)
    y = PartitionTree(adj_matrix=whole_bipartite_adj)
    y.build_encoding_tree(2)
    for nid, node in y.tree_node.items():
        # Only process leaf nodes (nodes without descendants)
        if node.children is None:  
            # The partition of a leaf node stores the original node index (e.g., claims nodes are 0~m-1, statements are m~m+n-1)
            original_node_index = node.partition[0]
            # Only retain claims nodes (original index less than m)
            if original_node_index < m:  
                # Calculate the structural entropy value
                entropy = y.path_entropy(nid, 0.0)  
                # Store the entropy value in the dictionary with the node ID as the key
                claims_node_entropy[nid] = entropy  
                # print(f"Claims Node ID: {nid}, Original Index: {original_node_index}, Entropy: {entropy}")
    print("Claims Node Structural Entropy Dictionary:", claims_node_entropy)
    return claims_node_entropy  
    
if __name__ == "__main__":
    random_matrix = np.random.randint(0, 2, size=(7, 5))
    main(random_matrix)