"""Predict model correctness using a linear classifier on embeddings."""
...
def get_p_ik(train_embeddings, is_false, eval_embeddings=None, eval_is_false=None):
    """Train a logistic regression classifier to predict model correctness from embeddings.
    
    Args:
        train_embeddings (list[torch.Tensor]): List of training embeddings (shape [N, D])
        is_false (list[float]): Binary labels indicating incorrect predictions (1=incorrect)
        eval_embeddings (list[torch.Tensor], optional): List of evaluation embeddings
        eval_is_false (list[float], optional): Evaluation set labels (1=incorrect)
    
    Returns:
        np.ndarray: Probabilities of incorrect predictions for the evaluation set
    """
