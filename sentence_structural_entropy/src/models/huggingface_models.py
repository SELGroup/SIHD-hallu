...
class StoppingCriteriaSub(StoppingCriteria):
    def __init__(self, stops, tokenizer, match_on="text", initial_length=None):
        ...
        Args:
            stops (list): List of patterns (strings or token lists) to trigger stopping
            tokenizer (PreTrainedTokenizer): Tokenizer for text <-> token conversion
            match_on (str): Matching mode ("text" for string matching, "tokens" for token ID matching)
            initial_length (int): Length of initial input tokens (for text matching offset)
        """
        super().__init__()
        self.stops = stops
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor):
        """Check if generation should stop based on current input IDs.
        
        Args:
            input_ids (torch.LongTensor): Generated token IDs so far
            scores (torch.FloatTensor): Model scores (required by parent class but unused)
        
        Returns:
            bool: True if any stop pattern is matched, False otherwise
        """
...
class HuggingfaceModel(BaseModel):
    ...
    def __init__(self, model_name, stop_sequences=None, max_new_tokens=None):
        """Initialize Hugging Face model with specified configuration.
        
        Args:
            model_name (str): Name of the model to load (supports "llama", "falcon", "phi" variants)
            stop_sequences (list): List of stop sequences to terminate generation
            max_new_tokens (int): Maximum number of new tokens to generate per prediction
        """
